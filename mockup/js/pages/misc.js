(function (TH) {
  ...
  TH.pages.comingSoon = (root, key, opts = {}) => {
    ...
    let status = '';
    if (scopeOnly || (on && !opts.path)) status = U.note('info', 'Chưa có mockup UI cho mục này', 'Mục thuộc Phase ' + phase + ' theo scope nhưng bộ PNG Phase 2 không có màn tương ứng – chỉ mô tả chức năng.');
    else if (!on && avail) status = U.note('warn', 'Phase ' + phase + ' đang tắt', 'Bật trong <b>Công cụ nâng cao</b> (cuối sidebar, chỉ Admin) để mở toàn bộ màn Phase 2. ' + (admin ? `<a class="link" data-act="enable">Bật Phase ${phase} ngay →</a>` : 'Chuyển sang vai trò Quản trị viên để bật.'));
    else if (!avail) status = U.note('info', 'Phase ' + phase + ' chưa có mockup', 'Công tắc Phase 3 bị khóa – bộ PNG Phase 3 chưa được dựng.');
