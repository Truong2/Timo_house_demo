...
(function (TH) {
  ...
  R.render = () => {
    ...
    } else try { if (TH.phase && TH.phase.on(2) && TH.actions.expireHolds) TH.actions.expireHolds(); if (TH.actions.activateDueAssignments) TH.actions.activateDueAssignments(); m.route.handler(root, m.params, query); TH.auth.enforceUI(root, path); if (TH.ui.enhanceSideCards) TH.ui.enhanceSideCards(root); if (TH.layout.syncPeriod) TH.layout.syncPeriod(); } catch (e) { console.error(e); root.innerHTML = `<div class="card"><div class="card-b"><div class="note-box danger">${TH.icon('alert-triangle')}<div><b>Lỗi hiển thị trang</b>${TH.esc(e.message)}</div></div></div></div>`; }
