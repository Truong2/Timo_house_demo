/* TimoHouse mockup – parser trích xuất hợp đồng theo template "HỢP ĐỒNG CHO THUÊ PHÒNG" (HĐ mới của Linh, 4 trang).
   Đầu vào: text đã đọc từ PDF điền trên máy (pdf.js) hoặc file .txt; ảnh JPG/PNG được mô phỏng bằng file mẫu (engine mock-1, spec v1.8 §12.8).
...
(function (TH) {
  ...
  P.FIELDS = [
    ...
    D('price', 'Giá thuê/tháng (VND)', 'money', 'money', { req: true, money: true }), D('priceWords', 'Bằng chữ (giá thuê)', 'money'), D('paidBefore', 'Đã thanh toán trước khi ký (VND)', 'money', 'money', { money: true }), D('paidAtSign', 'Đóng thêm khi ký (VND)', 'money', 'money', { money: true }), D('elecIndex', 'Số điện', 'money', 'number'), D('waterIndex', 'Số nước', 'money', 'number'), D('deposit', 'Tiền đặt cọc (VND)', 'money', 'money', { req: true, money: true }), D('depositWords', 'Bằng chữ (tiền cọc)', 'money'), D('payDay', 'Ngày gửi thông báo', 'money', 'select', { req: true }), D('payFrom', 'Thanh toán từ ngày', 'money', 'number'), D('payTo', 'Thanh toán đến ngày', 'money', 'number'), D('bankAccount', 'Tài khoản nhận', 'money'), D('bankName', 'Ngân hàng', 'money'), D('bankAccountName', 'Chủ tài khoản', 'money'), D('bankAccountNo', 'Số tài khoản', 'money'), D('transferNote', 'Nội dung chuyển khoản', 'money'), D('lateFee', 'Phạt chậm trả (đ/ngày)', 'money', 'money'), D('renewalMonths', 'Tự gia hạn (tháng)', 'term', 'number'), D('noticeDays', 'Báo trước (ngày)', 'term', 'number'), D('purpose', 'Mục đích thuê', 'room'), D('note', 'Ghi chú', 'money'),
  ...
  /* §12.8.4 entity_group: 13 nhóm review (§12.8.7) – field path → nhóm entity đích */
  P.ENTITY_GROUPS = [['CUSTOMER', 'Khách hàng', 'user'], ['BUILDING', 'Tòa nhà', 'building'], ['ROOM', 'Phòng', 'home'], ['CONTRACT', 'Hợp đồng', 'file-text'], ['OCCUPANT', 'Người ở', 'users'], ['CONTRACT_SERVICE', 'Dịch vụ & Giá', 'sliders'], ['DEPOSIT', 'Cọc', 'shield'], ['VEHICLE', 'Xe', 'car'], ['METER_READING', 'Điện/Nước đầu kỳ', 'zap'], ['HANDOVER_ASSET', 'Tài sản bàn giao', 'package'], ['PAYMENT_TERM', 'Điều khoản thanh toán', 'credit-card'], ['RENEWAL_CLAUSE', 'Gia hạn / Báo trước', 'refresh'], ['DOCUMENT', 'Tài liệu', 'folder']];
  const EG = { CUSTOMER: ['tenantName', 'dob', 'phone', 'idNumber', 'idPlace', 'address'], BUILDING: ['buildingName', 'signPlace', 'landlordName', 'landlordId', 'landlordIdPlace', 'landlordAddress', 'landlordPhone'], ROOM: ['roomCode'], CONTRACT: ['contractNo', 'signDate', 'purpose', 'occupants', 'maxOccupants', 'handoverDate', 'billingStart', 'start', 'end', 'months', 'price', 'priceWords', 'paidBefore', 'paidAtSign', 'note'], CONTRACT_SERVICE: ['svc_internet', 'svc_electric', 'svc_water', 'svc_common', 'svc_ebike', 'svc_evcar'], DEPOSIT: ['deposit', 'depositWords'], VEHICLE: ['vehicleCount'], METER_READING: ['elecIndex', 'waterIndex'], PAYMENT_TERM: ['payDay', 'payFrom', 'payTo', 'bankAccount', 'bankName', 'bankAccountName', 'bankAccountNo', 'transferNote', 'lateFee'], RENEWAL_CLAUSE: ['renewalMonths', 'noticeDays', 'noticeDate'] };
  P.entityGroupOf = (key) => Object.keys(EG).find(g => EG[g].includes(key)) || 'CONTRACT';
  P.fieldsOfGroup = (g) => EG[g] || [];
  /* Trang & vùng nguồn theo layout template 4 trang (mô phỏng bounding box theo % trang) */
  const PAGE = { 1: ['contractNo', 'signDate', 'signPlace', 'landlordName', 'landlordId', 'landlordIdPlace', 'landlordAddress', 'landlordPhone', 'tenantName', 'dob', 'phone', 'idNumber', 'idPlace', 'address', 'roomCode', 'buildingName', 'purpose', 'occupants', 'maxOccupants', 'vehicleCount'], 2: ['handoverDate', 'billingStart', 'months', 'start', 'end', 'noticeDate', 'renewalMonths', 'price', 'priceWords', 'paidBefore', 'paidAtSign', 'elecIndex', 'waterIndex', 'svc_internet', 'svc_electric', 'svc_water', 'svc_common', 'svc_ebike', 'svc_evcar'], 3: ['deposit', 'depositWords', 'payDay', 'payFrom', 'payTo', 'bankAccount', 'bankName', 'bankAccountName', 'bankAccountNo', 'transferNote', 'lateFee'], 4: ['noticeDays', 'note'] };
  P.pageOf = (key) => Number(Object.keys(PAGE).find(p => PAGE[p].includes(key)) || 1);
  P.bboxOf = (key) => { const p = P.pageOf(key); const idx = PAGE[p].indexOf(key); const n = PAGE[p].length || 1; return { x: 8, y: Math.round(6 + (idx / n) * 86), w: 84, h: Math.max(3, Math.round(86 / n)) }; };
  P.hashText = (text) => { const s = String(text || ''); let h1 = 0x811c9dc5, h2 = 0; for (let i = 0; i < s.length; i++) { h1 ^= s.charCodeAt(i); h1 = Math.imul(h1, 0x01000193) >>> 0; h2 = (h2 * 31 + s.charCodeAt(i)) | 0; } return ('00000000' + h1.toString(16)).slice(-8) + ('00000000' + (h2 >>> 0).toString(16)).slice(-8); };
  P.CONFIRM_AT = 0.8;
  ...
  P.parse = (text) => {
    ...
    const pu = rx(E1, /mục đích\s*(.*?)\s*(?=[.,]|Số lượng)/i); set('purpose', pu ? pu.replace(/^để\s*/i, 'Để ') : '', pu ? 0.9 : 0);
    /* Điều 3 */
    ...
    const bk = out.bankAccount.match(/^([A-Za-z]+)\s+(.*?)\s*:\s*([\d\s]+)$/); set('bankName', bk ? bk[1] : (out.bankAccount.split(/\s/)[0] || ''), bk ? 0.95 : out.bankAccount ? 0.6 : 0); set('bankAccountName', bk ? clean(bk[2]) : '', bk ? 0.95 : 0); set('bankAccountNo', bk ? digits(bk[3]) : digits((out.bankAccount.match(/\d[\d\s]{6,}/) || [''])[0]), bk ? 0.95 : 0.5);
    const pw = E4.match(/Từ ngày\s*(\d{1,2})\s*đến ngày\s*(\d{1,2})/i); set('payFrom', pw ? pw[1] : '', pw ? 0.95 : 0); set('payTo', pw ? pw[2] : '', pw ? 0.95 : 0);
    const lf = rx(t, /phạt\s*([\d.,]+)\s*đồng/i); set('lateFee', lf ? String(P.parseMoney(lf)) : '', lf ? 0.9 : 0);
    const rn = rx(t, /kéo dài thêm mỗi lần\s*(\d+)\s*tháng/i) || rx(t, /gia hạn\s*(\d+)\s*tháng/i); set('renewalMonths', rn, rn ? 0.9 : 0);
    const nd = rx(t, /báo trước[^.]{0,40}?ít nhất\s*(\d+)\s*ngày/i) || rx(t, /Trước\s*(\d+)\s*ngày hết hạn/i); set('noticeDays', nd, nd ? 0.9 : 0);
    ...
    /* Kết quả */
    const fields = P.FIELDS.map(d => { const value = out[d.key] == null ? '' : out[d.key]; const confidence = Math.round((conf[d.key] || 0) * 100) / 100; const eg = P.entityGroupOf(d.key); return { key: d.key, label: d.label, value, confidence, group: d.group, entityGroup: eg, money: !!d.money, raw: value, normalized: value, page: P.pageOf(d.key), bbox: P.bboxOf(d.key), confirmed: confidence >= P.CONFIRM_AT, decision: confidence >= P.CONFIRM_AT ? 'accept' : 'pending' }; });
  ...
  P.entities = (o) => {
    ...
    let match = null, matchBy = null, weak = false, conflicts = [], missing = [], diffs = [];
    if (St && St.state) { const dec = (o.decisions || {}).CUSTOMER; if (dec && dec.decision === 'link' && dec.candidateId && St.rawGet('tenants', dec.candidateId)) { match = St.rawGet('tenants', dec.candidateId); matchBy = 'chọn'; } else if (o.tenantId && St.get('tenants', o.tenantId)) { match = St.get('tenants', o.tenantId); matchBy = 'chọn'; } else if (TH.q && TH.q.tenantMatch) { const r = TH.q.tenantMatch(tenantData); match = r.match; matchBy = r.matchBy; conflicts = r.conflicts || []; missing = r.missing || []; diffs = r.diffs || []; if (!match && r.weak) { match = r.weak; matchBy = 'tên'; weak = true; } } }
