...
(function (TH) {
  ...
  X.releaseHold = (roomId, reason) => { const h = Q.roomHold(roomId); if (h) St.remove('holds', h.id); const r = Q.room(roomId); if (r.status === 'held') St.update('rooms', roomId, { status: 'ready' }); St.audit('release_hold', 'room', roomId, 'Hủy giữ chỗ phòng ' + r.code + (h ? ' (' + Q.tenant(h.tenantId).name + ')' : '') + (reason ? ' – ' + reason : '')); done(); };
  ...
  X.activateContract = (id, key) => idem(key, () => {
    ...
    const hold = Q.roomHold(room.id); if (hold && hold.tenantId !== c.tenantId) err('Phòng ' + room.code + ' đang giữ chỗ cho khách ' + Q.tenant(hold.tenantId).name + ' – hủy giữ chỗ trước'); if (hold) { St.remove('holds', hold.id); St.audit('release_hold', 'room', room.id, 'Giữ chỗ chuyển thành hợp đồng ' + c.code); }
  ...
  X.saveRefund = (d) => {
    ...
    let rf; const existing = d.id ? St.get('refunds', d.id) : St.one('refunds', r => r.contractId === d.contractId && ['draft', 'rejected'].includes(r.status));
    if (existing && !['draft', 'rejected'].includes(existing.status)) err('Hồ sơ ' + existing.code + ' đang ' + Q.label('refund', existing.status) + ' – không sửa được phương án');
    ...
    if (existing) { rf = existing; Object.assign(rf, base, { status: rf.status === 'rejected' ? 'draft' : rf.status }); } else rf = St.add('refunds', Object.assign({ code: St.nextCode('refunds', 'RC202410-', 3), status: 'draft', rejectReason: '' }, base));
  ...
  X.saveExpense = (d) => {
    ...
    let ex; const payload = { date: d.date, group: d.group, desc: d.desc || '', buildingId: d.buildingId || null, amount, recordType: d.recordType || (common ? 'common' : 'ops'), method: d.method || 'cash', evidence: d.evidence || '', note: d.note || '', status: common && !allocs.length ? 'pending_alloc' : 'recorded' };
  ...
  X.resumeBatch = (id) => {
    ...
    timers[id] = setInterval(() => {
      ...
      msgs.forEach((m, i) => {
        ...
        else if (m.status === 'accepted' && h < 50) { const r = parseInt(F.hash(m.id).slice(-2), 16) % 100; if (r < 88) m.status = 'delivered'; else if (r < 95) { m.status = 'failed'; m.errorCode = r < 92 ? 'ZLM-1001' : 'ZLM-2003'; } else m.status = 'unknown'; changed++; }
      });
  ...
  X.saveUser = (d) => {
    ...
    if (['sale', 'tech'].includes(d.role) && !d.id) err('Vai trò Sale/Kỹ thuật thuộc Phase 2 – bản demo chỉ tạo Quản trị viên / Kế toán / Vận hành');
