...
(function (TH) {
  ...
  X.createInvoiceDrafts = ({ period, contractIds, services, issueNow, key }) => idem(key, () => {
    ...
    contractIds.forEach(cid => {
      ...
      if (useE) { L('Điện', 'Tiền điện (' + (e.curr - e.prev) + ' kWh x ' + F.vnd(pE) + ')', e.curr - e.prev, pE, 'electric', { meterId: e.id, serviceId: S('DIEN').id }); e.status = 'used'; e.contractId = c.id; }
      if (useW) { L('Nước', 'Tiền nước (' + (w.curr - w.prev) + ' m³ x ' + F.vnd(pW) + ')', w.curr - w.prev, pW, 'water', { meterId: w.id, serviceId: S('NUOC').id }); w.status = 'used'; w.contractId = c.id; }
