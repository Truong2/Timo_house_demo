...
(function (TH) {
  ...
  function validateContract(d) {
    ...
    const clash = St.one('contracts', c => c.roomId === d.roomId && c.id !== d.id && c.id !== d.renewedFromId && ['active', 'pending_renewal', 'pending_end'].includes(c.status) && !(d.end < c.start || d.start > c.end)); if (clash) err('Phòng đã có hợp đồng ' + clash.code + ' hiệu lực trùng khoảng thuê (FR-CUS-02)');
  }
  ...
  X.activateContract = (id, key, opts = {}) => idem(key, () => {
    ...
    const prev = c.renewedFromId ? St.get('contracts', c.renewedFromId) : null; const renewing = !!(prev && ['active', 'pending_renewal'].includes(prev.status) && prev.roomId === c.roomId);
  ...
  X.saveService = (d) => {
    ...
    let s; if (d.id) { const old = St.get('services', d.id); const priceChanged = Number(old.price) !== Number(d.price); s = St.update('services', d.id, d); if (priceChanged) { St.add('priceHistory', { serviceId: s.id, date: d.effectiveFrom, price: Number(d.price), scope: d.scope === 'all' ? 'Tất cả tòa nhà' : (d.buildingIds || []).map(b => Q.building(b).name).join(', '), userId: me(), userName: (St.state.session || {}).name }); if (X.saveServicePrice) X.saveServicePrice({ serviceId: s.id, scope: 'GLOBAL', price: Number(d.price), effectiveFrom: d.effectiveFrom, reason: 'Đổi giá mặc định từ màn Dịch vụ', silent: true }); } }
    else { s = St.add('services', Object.assign({ code: F.slug(d.name).toUpperCase().replace(/-/g, '_'), group: 'Dịch vụ tiện ích', method: 'fixed', scope: 'all', buildingIds: [], effectiveTo: '', status: 'active', icon: 'package', note: '' }, d)); St.add('priceHistory', { serviceId: s.id, date: s.effectiveFrom, price: Number(s.price), scope: 'Tất cả tòa nhà', userId: me(), userName: (St.state.session || {}).name }); if (X.saveServicePrice) X.saveServicePrice({ serviceId: s.id, scope: 'GLOBAL', price: Number(s.price), effectiveFrom: s.effectiveFrom, reason: 'Giá mặc định khi tạo dịch vụ', silent: true }); }
  ...
  /* §12.9.2 / §12.8.10: giá điện/nước = snapshot trên HĐ (contractServices theo chỉ số) nếu có, ngược lại giá tòa hiệu lực cuối kỳ → giá mặc định */
  const meterPrice = (c, code, period) => { const s = St.one('services', x => x.code === code); if (!s) return 0; const cs = St.one('contractServices', x => x.contractId === c.id && x.serviceId === s.id && Number(x.price) > 0); if (cs) return Number(cs.price); const date = period ? (TH.q.periodEndDate ? TH.q.periodEndDate(period) : period + '-28') : F.today(); return TH.q.servicePrice ? TH.q.servicePrice(s.id, c.buildingId, date) : Number(s.price) || 0; };
  X.meterPrice = meterPrice;
  X.batchRows = (period, buildingIds) => {
    const svc = F.by(St.all('services'), 'code'); const S = c => (svc[c] || [{}])[0];
    return St.where('contracts', c => ['active', 'pending_renewal', 'pending_end'].includes(c.status) && c.start <= period + '-31' && (!buildingIds || !buildingIds.length || buildingIds.includes(c.buildingId))).map(c => {
      ...
      return { contract: c, room, invoice: inv, electricPrev: curE ? curE.prev : (rE.usedPrev != null ? rE.usedPrev : (prevE ? prevE.curr : 0)), electricCurr: curE ? curE.curr : null, waterPrev: curW ? curW.prev : (rW.usedPrev != null ? rW.usedPrev : (prevW ? prevW.curr : 0)), waterCurr: curW ? curW.curr : null, rent: c.price, fixed, warns, priceE: meterPrice(c, 'DIEN', period), priceW: meterPrice(c, 'NUOC', period), valid: !inv && curE && curW && curE.curr >= curE.prev && curW.curr >= curW.prev };
  ...
  X.createInvoiceDrafts = ({ period, contractIds, services, issueNow, key }) => idem(key, () => {
    ...
    contractIds.forEach(cid => {
      const c = St.get('contracts', cid); if (!c || !['active', 'pending_renewal', 'pending_end', 'pending_settlement'].includes(c.status)) return;
      ...
      const inv = St.add('invoices', { code: St.nextCode('invoices', 'HD-' + period.replace('-', '') + '-', 3), contractId: c.id, roomId: c.roomId, buildingId: c.buildingId, tenantId: c.tenantId, period, issueDate: null, dueDate: due, docStatus: 'draft', total: 0, adjustment: 0, carriedOver: Q.contractDebt(c.id), note: '', createdBy: me(), reminderCount: 0, lastReminderAt: null, accountingNote: '', snapshot: { contractCode: c.code, price: c.price, tenantName: Q.tenant(c.tenantId).name, roomCode: Q.room(c.roomId).code } });
      let seq = 1; const LT = { rent: 'RENT', service: 'SERVICE', electric: 'SERVICE', water: 'SERVICE' }; const L = (item, desc, qty, unitPrice, kind, extra = {}) => St.add('invoiceLines', Object.assign({ invoiceId: inv.id, seq: seq++, item, desc, qty, unitPrice, amount: qty * unitPrice, kind, lineType: LT[kind] || 'OTHER', unit: kind === 'rent' ? 'tháng' : kind === 'electric' ? 'kWh' : kind === 'water' ? 'm³' : (extra.unit || '') }, extra));
      ...
      const pE = meterPrice(c, 'DIEN', period), pW = meterPrice(c, 'NUOC', period);
      if (useE) { L('Điện', 'Tiền điện (' + (e.curr - e.prev) + ' kWh x ' + F.vnd(pE) + ')', e.curr - e.prev, pE, 'electric', { meterId: e.id, sourceReadingId: e.id, serviceId: S('DIEN').id }); e.status = 'used'; e.contractId = c.id; }
      if (useW) { L('Nước', 'Tiền nước (' + (w.curr - w.prev) + ' m³ x ' + F.vnd(pW) + ')', w.curr - w.prev, pW, 'water', { meterId: w.id, sourceReadingId: w.id, serviceId: S('NUOC').id }); w.status = 'used'; w.contractId = c.id; }
  ...
  X.addInvoiceLine = (id, { item, desc, qty, unitPrice, lineType = 'SERVICE', serviceId = null, unit = '' }) => { const inv = St.get('invoices', id); if (inv.docStatus !== 'draft') err('Chỉ thêm dòng cho hóa đơn Nháp'); req(item, 'Tên khoản'); const q = F.num(qty) || 1, up = F.num(unitPrice); if (!up) err('Đơn giá khác 0'); if (lineType === 'ADJUSTMENT' && !desc) err('Điều chỉnh phải có diễn giải/chứng từ đối ứng'); const l = St.add('invoiceLines', { invoiceId: id, seq: Q.invLines(id).length + 1, item, desc: desc || item, qty: q, unitPrice: up, amount: q * up, kind: lineType === 'RENT' ? 'rent' : 'service', lineType, serviceId, unit }); inv.total = F.sum(Q.invLines(id), x => x.amount); St.audit('add_line', 'invoice', id, 'Thêm dòng "' + item + '" ' + F.vnd(q * up) + ' vào ' + inv.code); done(); return l; };
  ...
  /* ---------- Hoàn cọc ---------- */
  X.saveRefund = (d) => {
    req(d.contractId, 'Chọn hợp đồng'); const c = Q.contract(d.contractId); if (!['ended', 'active', 'pending_end', 'pending_settlement', 'pending_renewal'].includes(c.status)) err('Hợp đồng không hợp lệ');
    ...
    const debt = Q.contractDebt(c.id);
    const base = { contractId: c.id, tenantId: c.tenantId, roomId: c.roomId, buildingId: c.buildingId, requestDate: F.today(), moveOutDate: d.moveOutDate || c.actualEnd || c.end, deposit: d.depositHeld || (Q.depositBalance && Q.depositBalance(c.id).holding) || c.deposit, debt, offsetDebt: !!d.offsetDebt, debtOffset: d.offsetDebt ? (d.debtOffset != null && d.debtOffset !== '' ? Math.min(F.num(d.debtOffset), debt) : debt) : 0, assetCheck: d.assetCheck || (d.id ? (St.get('refunds', d.id) || {}).assetCheck : []) || [], endType: d.endType || c.endType || '', approverRole: d.approverRole || 'Kế toán trưởng', inspection: d.inspection || {}, files: d.files || [], handlerId: me() };
