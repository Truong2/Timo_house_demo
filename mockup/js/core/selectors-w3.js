...
(function (TH) {
  ...
  Q.meterRows = (period, f = {}) => {
    ...
    return rooms.map(r => { const c = raw('contracts').find(x => x.roomId === r.id && Q.contractLive(x) && x.start <= period + '-31'); const mk = (type) => { const isP = (m) => !m.readingType || m.readingType === 'PERIOD'; const cur = raw('meterReadings').filter(m => m.roomId === r.id && m.type === type && m.period === period && isP(m)).sort((a, b) => F.cmp(b.createdAt || '', a.createdAt || ''))[0] || null; const prevList = raw('meterReadings').filter(m => m.roomId === r.id && m.type === type && ((m.period && m.period < period) || m.readingType === 'OPENING') && m.status !== 'reopened').sort((a, b) => F.cmp(b.period || b.readingDate || '', a.period || a.readingDate || '')); const prev = prevList[0] || null; const prevUse = prevList[0] && prevList[1] ? Number(prevList[0].curr) - Number(prevList[1].curr) : null; const meter = Q.meter(r.id, type); const use = cur ? Number(cur.curr) - Number(cur.prev) : null; const issues = []; if (cur && Number(cur.curr) < Number(cur.prev)) issues.push('new<old'); if (!cur && c) issues.push('missing'); const dupes = raw('meterReadings').filter(m => m.roomId === r.id && m.type === type && m.period === period && isP(m) && m.status !== 'reopened').length; if (dupes > 1) issues.push('duplicate'); if (cur && prevUse != null && prevUse > 0 && use != null && Math.abs(use - prevUse) / prevUse > 0.5) issues.push('abnormal'); return { type, meter, cur, prev, prevValue: cur ? Number(cur.prev) : (prev ? Number(prev.curr) : 0), use, prevUse, issues }; }; return { room: r, building: Q.building(r.buildingId), contract: c || null, tenant: c ? Q.tenant(c.tenantId) : null, electric: mk('electric'), water: mk('water') }; }).sort((a, b) => F.cmp(a.building.name, b.building.name) || F.cmp(a.room.code, b.room.code));
  };
