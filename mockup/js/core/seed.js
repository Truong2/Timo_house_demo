...
(function (TH) {
  ...
  seed.run = (st) => {
    ...
    const mk = (col, obj) => { obj.id = obj.id || F.uid(col.slice(0, 3)); obj.source = 'seed'; obj.createdAt = obj.createdAt || '2024-10-01T08:00'; st[col].push(obj); return obj; };
    ...
    /* ---------- Chủ nhà & Tòa ---------- */
    const LL = [
      ['CN001', 'Trần Văn Hải', 'person', 'hai.tran@gmail.com', 3, '2024-01-01', '2025-12-31', 120000000, 'active', '2024-11-15'],
      ['CN002', 'Lê Thị Mai', 'person', 'mai.le@gmail.com', 6, '2023-06-01', '2024-12-31', 95000000, 'expiring', '2024-12-12'],
      ['CN003', 'Lê Văn Thành', 'person', 'thanh.le@gmail.com', 3, '2023-01-01', '2026-01-01', 88000000, 'active', '2024-11-07'],
      ['CN004', 'Công ty CP Đầu tư Bà Triệu', 'company', 'lienhe@batrieu.vn', 4, '2024-01-01', '2026-12-31', 80000000, 'active', '2025-01-11'],
      ['CN005', 'Bùi Thị Hạnh', 'person', 'hanh.bui@gmail.com', 3, '2024-03-01', '2027-02-28', 52000000, 'active', '2024-11-19'],
      ['CN006', 'Vũ Thị Mai', 'person', 'mai.vu@gmail.com', 3, '2024-01-01', '2025-12-31', 70000000, 'active', '2024-12-27'],
      ['CN007', 'Ngô Thị Quỳnh', 'person', 'quynh.ngo@gmail.com', 6, '2023-09-01', '2025-08-31', 60000000, 'paused', '2025-02-25'],
      ['CN008', 'Trần Văn Hưng', 'person', 'hung.tran@gmail.com', 3, '2024-01-01', '2026-12-31', 120000000, 'active', '2025-01-01'],
      ['CN009', 'Phạm Thị Lan', 'person', 'lan.pham@gmail.com', 4, '2023-05-01', '2025-04-30', 45000000, 'paused', '2025-01-15'],
      ['CN010', 'Công ty TNHH Đại Dương', 'company', 'info@daiduong.vn', 6, '2022-01-01', '2024-12-31', 65000000, 'paused', '2024-12-31'],
    ];
    const BD = [
    ...
    function addContract(o) {
      ...
      const c = mk('contracts', Object.assign({ code: 'HD-2024-' + F.pad(cCounter++, 3), buildingId: room.buildingId, listPrice: room.price, price: room.price, deposit: room.price * 2, cycle: 'monthly', payDay: rint(5, 20), status: 'active', managerId: room.managerId, signedDate: null, note: '', renewedFromId: null, renewedToId: null }, o));
    ...
    const H = [
      ...
      ['Trần Minh Đức', 'A.12.03', '2024-01-01', '2024-12-31', 12000000, 12000000, 22000000, 10],
      ['Nguyễn Thị Hương', 'B.05.01', '2024-01-16', '2025-01-15', 8500000, 8500000, 17000000, 16],
      ['Lê Quang Huy', 'C.08.02', '2024-01-01', '2024-12-31', 7000000, 7000000, 14000000, 18],
      ['Hoàng Nam Khánh', 'D.03.06', '2023-11-01', '2024-10-31', 5000000, 5000000, 5000000, 22],
      ['Trần Văn Nam', 'B.05.02', '2023-12-16', '2024-12-15', 7000000, 7000000, 14000000, 15],
      ['Vũ Thảo Nguyên', 'A.09.01', '2024-03-01', '2025-02-28', 9000000, 9000000, 18000000, 25],
      ['Đặng Quốc Bảo', 'B.12.05', '2024-02-01', '2025-01-31', 11000000, 11000000, 22000000, 28],
      ['Ngô Yến Nhi', 'C.06.10', '2024-04-01', '2025-03-31', 7500000, 7500000, 15000000, 30],
      ['Phan Văn Long', 'D.10.03', '2024-05-01', '2025-04-30', 10000000, 10000000, 20000000, 5],
      ['Bùi Thị Mai', 'A.04.07', '2024-06-01', '2025-05-31', 8000000, 8000000, 16000000, 5],
      ['Nguyễn Thảo Vy', 'A.11.02', '2024-03-01', '2025-02-28', 4800000, 4500000, 9000000, 10],
    ];
    const heroC = {};
    H.forEach(h => {
    ...
    occ.forEach((room, k) => {
      ...
      let start = F.addMonths('2024-10-01', -rint(1, 16)); start = start.slice(0, 8) + F.pad(rint(1, 28));
      ...
      else if (k < 32) { start = '2024-10-' + F.pad(rint(1, 25)); end = F.addDays(F.addMonths(start, 12), -1); } // cọc mới trong kỳ
    ...
    // Kỳ trước (08, 09/2024) cho tất cả HĐ đang hiệu lực – đã thu đủ, không sinh dòng chỉ số quá nhiều: chỉ hero
    Object.values(heroC).forEach(c => { ['2024-07', '2024-08', '2024-09'].forEach(p => { if (c.start <= p + '-01' && c.status === 'active') { const inv = makeInvoice(c, p); pay(inv, inv.total, F.addDays(inv.dueDate, -rint(0, 5))); } }); });
    ['2024-03', '2024-04', '2024-05', '2024-06'].forEach(p => { const c = heroC['Nguyễn Thảo Vy']; const inv = makeInvoice(c, p); pay(inv, inv.total, F.addDays(inv.dueDate, -2)); });
    // Kỳ 10/2024
    invN = 1;
    const heroInv = {};
    ...
    H.forEach(h => { const c = heroC[h[0]]; const inv = makeInvoice(c, '2024-10'); heroInv[h[0]] = inv; const pl = heroPlan[h[0]]; if (pl[0] === 'paid') pay(inv, inv.total, F.addDays(inv.dueDate, -1)); if (pl[0] === 'partial') pay(inv, pl[1], F.addDays(inv.dueDate, -1)); });
    heroInv['Lê Quang Huy'].reminderCount = 2; heroInv['Lê Quang Huy'].lastReminderAt = '2024-10-12T09:15'; heroInv['Lê Quang Huy'].accountingNote = 'Khách thuê đề nghị giữ nguyên mức phí. Kiểm tra chỉ số điện nước hàng tháng.';
    heroInv['Trần Minh Đức'].reminderCount = 1; heroInv['Trần Minh Đức'].lastReminderAt = '2024-10-20T09:00';
    // HĐ đã kết thúc Phạm Thu Trang: hóa đơn 10/2024 chưa thu
    makeInvoice(heroC['Phạm Thu Trang'], '2024-10');
    const rest = active.filter(c => !Object.values(heroC).includes(c));
    rest.forEach((c, k) => {
      const m = k % 20;
      if (m < 10) { const inv = makeInvoice(c, '2024-10'); pay(inv, inv.total, F.addDays(inv.dueDate, -rint(0, 6))); }
      else if (m < 12) { const inv = makeInvoice(c, '2024-10'); pay(inv, Math.round(inv.total * pick([.3, .5, .6]) / 100000) * 100000, F.addDays(inv.dueDate, -1)); }
      else if (m < 15) { makeInvoice(c, '2024-10'); }
      else if (m < 17) { makeInvoice(c, '2024-10', { draft: true, noMeter: k % 2 === 0 }); }
      else if (m < 18) { const inv = makeInvoice(c, '2024-10', { adjusted: true }); pay(inv, inv.total, F.addDays(inv.dueDate, -1)); }
    ...
    // Hero: Phạm Thu Trang A.16.08 (HĐ đã kết thúc 20/10, hóa đơn 10/2024 còn nợ) → hồ sơ Chờ duyệt
    ...
    /* ---------- Chi phí ---------- */
    const EX = [
      ['2024-10-02', 'Điện nước', 'Tiền điện tháng 10/2024', 'TH-HBT-01', 4500000, 'ops', 'HD001.pdf', 'vanhanh'],
      ['2024-10-04', 'Sửa chữa', 'Sửa máy lạnh phòng A.12.03', 'TH-HBT-01', 2800000, 'ops', 'BC002.jpg', 'lequanghuy'],
      ['2024-10-05', 'Marketing', 'Quảng cáo Facebook tháng 10', null, 3200000, 'common', 'INV003.pdf', 'admin'],
      ['2024-10-08', 'Mua sắm TS', 'Mua tủ lạnh cho phòng B.05.01', 'TH-PXL-02', 12500000, 'asset', 'HD004.pdf', 'lequanghuy'],
      ['2024-10-10', 'Thuê nhà', 'Chi phí thuê văn phòng tháng 10', null, 15000000, 'common', 'HD005.pdf', 'ketoan'],
      ['2024-10-12', 'Điện nước', 'Tiền nước tháng 10/2024', 'TH-Q1-03', 3600000, 'ops', 'HD006.pdf', 'lequanghuy'],
      ['2024-10-14', 'Sửa chữa', 'Thay bóng đèn khu hành lang', 'TH-BTD-04', 1200000, 'ops', 'BC007.jpg', 'hnkhanh'],
      ['2024-10-15', 'Mua sắm TS', 'Mua máy giặt công nghiệp', null, 28000000, 'asset', 'HD008.pdf', 'admin'],
      ['2024-10-18', 'Marketing', 'Thiết kế banner, tờ rơi', null, 2000000, 'common', 'INV009.pdf', 'admin'],
      ['2024-10-20', 'Sửa chữa', 'Sửa cửa phòng C.10.01', 'TH-Q1-03', 1600000, 'ops', 'BC010.jpg', 'lequanghuy'],
    ];
    const EXG = ['Điện nước', 'Sửa chữa', 'Marketing', 'Mua sắm TS', 'Thuê nhà', 'Lương', 'Khác'];
    for (let k = 0; k < 28; k++) {
      const e = EX[k] || [F.addDays('2024-10-01', rint(0, 27)), pick(EXG), pick(['Bảo trì thang máy', 'Vệ sinh định kỳ', 'Mua vật tư sửa chữa', 'Phí internet tòa', 'Rác & môi trường', 'Lương NV vệ sinh']), pick(BD.slice(0, 5))[0], rint(5, 80) * 100000, pick(['ops', 'ops', 'common']), 'CT' + F.pad(k + 1, 3) + '.pdf', pick(['vanhanh', 'ketoan', 'admin'])];
    ...
    const ZB = [
      ['Nhắc tiền phòng T10', 'reminder', 'Khách thuê', 320, 302, 12, '2024-10-22T09:00', 'admin', 'sending'],
      ['Nhắc lịch xem phòng', 'p2_viewing', 'Khách quan tâm', 86, 86, 0, '2024-10-20T14:30', 'ttmai', 'done'],
      ['HĐ sắp hết hạn T10', 'contract_expiry', 'Khách thuê', 124, 118, 6, '2024-10-18T09:00', 'lequanghuy', 'partial'],
      ['Thông báo bảo trì thang máy', 'broadcast', 'Cư dân tòa nhà', 280, 270, 10, '2024-10-16T10:00', 'pttrang', 'partial'],
      ['Nhắc tiền phòng T9 (lần 2)', 'reminder', 'Khách thuê', 315, 290, 25, '2024-10-12T09:00', 'admin', 'failed'],
      ['Chúc mừng sinh nhật', 'p2_care', 'Khách thuê', 42, 41, 1, '2024-10-10T08:00', 'vtmai', 'done'],
      ['Nhắc lịch xem phòng', 'p2_viewing', 'Khách quan tâm', 67, 62, 5, '2024-10-08T14:00', 'dqbao', 'partial'],
      ['Nhắc HĐ sắp hết hạn', 'contract_expiry', 'Khách thuê', 58, 0, 58, '2024-10-05T09:00', 'nvan', 'failed'],
      ['Thông báo điều chỉnh giá điện', 'broadcast', 'Cư dân tòa nhà', 410, 395, 15, '2024-10-02T10:00', 'admin', 'partial'],
      ['Nhắc tiền phòng T9', 'reminder', 'Khách thuê', 298, 298, 0, '2024-09-22T09:00', 'admin', 'done'],
    ];
    const ERRS = ['ZLM-1001', 'ZLM-2003', 'ZLM-1001', 'ZLM-3005'];
    for (let k = 28; k >= 1; k--) {
      const z = ZB[28 - k] || [pick(['Nhắc tiền phòng', 'Nhắc công nợ', 'Thông báo chung']), pick(['reminder', 'debt', 'broadcast']), 'Khách thuê', rint(40, 300), 0, 0, F.addDays('2024-09-01', rint(0, 40)) + 'T09:00', 'admin', 'done'];
      if (!ZB[28 - k]) { z[4] = z[3] - rint(0, 8); z[5] = z[3] - z[4]; z[8] = z[5] ? 'partial' : 'done'; }
      const tk = tpl[z[1]] ? tpl[z[1]][0] : tpl['reminder'][0];
      mk('zaloBatches', { code: 'ZL-202410-' + F.pad(k, 3), name: z[0], eventKey: z[1], audience: z[2], plannedCount: z[3], sentCount: z[4], failedCount: z[5], sentAt: z[6], createdAt: z[6], createdBy: uid(z[7]), status: z[8], templateId: tk.id, source: 'seed', sourceKey: z[1] === 'reminder' ? 'due_soon' : z[1] === 'debt' ? 'overdue' : z[1], scopeLabel: pick(['Tất cả tòa nhà', 'Tòa Sunrise', 'Tòa Moonlight', 'Khu Thủ Đức']), period: '2024-10', sendMode: 'now', rules: 'Gửi cho hóa đơn còn nợ > 0 · Chỉ gửi 1 tin / khách / kỳ · Bỏ qua nếu khách đã nhận tin trong 3 ngày qua', errors: z[5] ? pick(['Lỗi không tìm thấy số điện thoại', 'Khách hàng chặn tin nhắn', 'Lỗi kết nối Zalo OA', 'Số điện thoại không hợp lệ', 'Vượt giới hạn gửi trong ngày']) : '' });
    }
    // Tin nhắn của đợt 028 (đang gửi) từ hóa đơn 10/2024 đã phát hành
    const b28 = st.zaloBatches.find(b => b.code === 'ZL-202410-028');
    const issued = st.invoices.filter(x => x.period === '2024-10' && x.docStatus !== 'draft');
  ...
  function addUsers(st) {
    ...
    const mk = (o) => { o.id = F.uid('usr'); o.source = 'seed'; o.createdAt = '2024-01-01T08:00'; st.users.push(o); return o; };
    const U = [
      ['admin', 'Nguyễn Văn Minh', 'minhnv@timohouse.vn', '0901 234 567', 'admin', 'Toàn hệ thống', 'active', '2024-10-22T14:30'],
      ['ketoan', 'Trần Thị Lan', 'lantt@timohouse.vn', '0902 345 678', 'accountant', 'Tài chính', 'active', '2024-10-22T09:15'],
      ['vanhanh', 'Nguyễn Thị Hương', 'huongnt@timohouse.vn', '0903 456 789', 'ops', 'Tòa Sunrise', 'active', '2024-10-21T16:45'],
      ['lequanghuy', 'Lê Quang Huy', 'huylq@timohouse.vn', '0904 567 890', 'ops', 'Tòa Moonlight, Tòa Riverside', 'active', '2024-10-22T08:20'],
      ['hnkhanh', 'Hoàng Nam Khánh', 'khanhhn@timohouse.vn', '0905 678 901', 'ops', 'Tòa Central', 'active', '2024-10-20T11:10'],
      ['tranminhduc', 'Trần Minh Đức', 'ductm@timohouse.vn', '0906 789 012', 'ops', 'Tòa Garden', 'active', '2024-10-22T07:55'],
  ...
  function addCatalog(st) {
    ...
    const mk = (col, o) => { o.id = F.uid(col.slice(0, 3)); o.source = 'seed'; o.createdAt = '2024-01-01T08:00'; st[col].push(o); return o; };
    [
      ...
      const o = mk('services', { code: s[0], name: s[1], group: s[2], unit: s[3], method: s[4], price: s[5], icon: s[6], scope: s[0] === 'GUIXE' ? 'buildings' : 'all', buildingIds: [], effectiveFrom: '2024-01-01', effectiveTo: '', status: 'active', note: s[7] });
      mk('priceHistory', { serviceId: o.id, date: '2024-01-01', price: s[5], scope: 'Tất cả tòa nhà', userId: null, userName: 'Nguyễn Văn Minh' });
      if (s[0] === 'INTERNET') { mk('priceHistory', { serviceId: o.id, date: '2023-06-01', price: 180000, scope: 'Tất cả tòa nhà', userName: 'Trần Thị Lan' }); mk('priceHistory', { serviceId: o.id, date: '2023-01-01', price: 150000, scope: 'Tất cả tòa nhà', userName: 'Nguyễn Văn Minh' }); }
    });
