...
(function (TH) {
  ...
  L.MENU = [
    ...
    { key: 'operations', group: 'Vận hành', icon: 'building', items: [
      ...
      { key: 'maintenance', label: 'Bảo trì - Sửa chữa', icon: 'wrench', p: 'P2' },
      { key: 'tasks', label: 'Lịch công việc', icon: 'calendar', p: 'P2' },
    ] },
    { key: 'finance', group: 'Tài chính', icon: 'wallet', items: [
      ...
      { key: 'cashbook', label: 'Thu chi', icon: 'arrow-left-right', p: 'P2' },
      { key: 'deposits', label: 'Quản lý cọc', icon: 'piggy', p: 'P2' },
      { key: 'bank', label: 'Ngân hàng', icon: 'landmark', p: 'P3' },
    ] },
    ...
    { key: 'business', group: 'Kinh doanh', icon: 'briefcase', items: [
      { key: 'crm', label: 'Khách hàng', icon: 'user', p: 'P2' }, { key: 'channels', label: 'Kênh cho thuê', icon: 'share', p: 'P2' }, { key: 'viewings', label: 'Lịch xem phòng', icon: 'calendar-check', p: 'P2' }, { key: 'marketing', label: 'Marketing', icon: 'megaphone', p: 'P2' },
    ] },
    { key: 'hr', group: 'Nhân sự', icon: 'users', items: [{ key: 'hr', label: 'Nhân viên', icon: 'users', p: 'P3' }, { key: 'timesheet', label: 'Chấm công', icon: 'clock', p: 'P3' }, { key: 'payroll', label: 'Lương thưởng', icon: 'banknote', p: 'P3' }] },
    { key: 'investment', group: 'Đầu tư', icon: 'trending-up', items: [{ key: 'projects', label: 'Dự án', icon: 'folder', p: 'P3' }, { key: 'assets', label: 'Tài sản', icon: 'package', p: 'P3' }, { key: 'roi', label: 'Hiệu quả đầu tư', icon: 'trending-up', p: 'P3' }] },
    { key: 'reports', group: 'Báo cáo', icon: 'bar-chart', items: [{ key: 'reports', label: 'Báo cáo Phase 1', icon: 'bar-chart', href: '#/reports', permission: 'reports.view' }, { key: 'reports-detail', label: 'Báo cáo chi tiết', icon: 'list', p: 'P2', permission: 'reports.view' }] },
    { key: 'settings', group: 'Cấu hình', icon: 'settings', items: [
      ...
      { key: 'system', label: 'Thiết lập hệ thống', icon: 'settings', p: 'P2', permission: 'advancedTools' },
    ] },
  ];
  ...
  const visibleItems = group => group.items.filter(it => !it.permission || TH.auth.can(it.permission));
  const sidebarItem = (it) => `<a class="sb-item ${it.p ? it.p.toLowerCase() : ''}" data-key="${it.key}" href="${it.href || '#/coming-soon/' + it.key}">${I(it.icon)}<span>${esc(it.label)}</span>${it.p ? `<span class="ptag">${it.p}</span>` : ''}</a>`;
  ...
  L.P2_INFO = {
    ...
    channels: ['Kênh cho thuê', 2, 'Quản lý kênh đăng tin, đối tác môi giới, hiệu quả kênh.', 'FR-SAL-01/05'],
    viewings: ['Lịch xem phòng', 2, 'Đặt lịch xem phòng, nhân viên phụ trách, nhắc lịch, kết quả xem.', 'FR-SAL-02'],
    marketing: ['Marketing', 2, 'Chiến dịch, chi phí marketing, đo hiệu quả theo nguồn.', 'FR-SAL-05'],
    ...
    system: ['Thiết lập hệ thống', 2, 'Thông số hệ thống, múi giờ, làm tròn, chính sách lưu trữ (OI-24).', 'NFR'],
  ...
  const openUserMenu = el => {
    ...
    if (s.role === 'admin' || s.impersonator) {
      ...
      ['admin', 'accountant', 'ops'].forEach(role => items.push({ label: role === 'admin' && s.impersonator ? 'Quay lại Admin' : (s.role === role ? '● ' : '○ ') + TH.auth.ROLE_LABEL[role], icon: role === 'admin' && s.impersonator ? 'corner-up-left' : 'user', onClick: () => switchDemoRole(role) }));
      items.push('-');
    }
  ...
  L.ensure = (app) => {
    ...
    app.innerHTML = `<div class="app"><div class="sb-backdrop" data-act="sb-close"></div><aside class="sidebar"><div class="sb-logo"><div class="mark">${I('home')}</div><div><div class="name">Timo<span>House</span></div><div class="tag">Quản lý nhà cho thuê</div></div></div><nav class="sb-nav" data-guide="sidebar">${L.MENU.map(sidebarGroup).join('')}</nav><div class="sb-foot"><div>Phiên bản 1.0.0 · Mockup Phase 1</div><div>TimoHouse © 2026</div><details><summary>${I('settings')} Công cụ nâng cao</summary><div class="adv"><button data-act="adv-reset">↺ Đặt lại dữ liệu demo</button><button data-act="adv-clear">✕ Xóa trắng dữ liệu nghiệp vụ</button><button data-act="adv-today">📅 Đổi ngày hệ thống demo</button><button data-act="adv-export">⇩ Xuất state JSON</button><button data-act="adv-import">⇧ Nhập state JSON</button><button data-act="adv-runall">▶ Chạy toàn bộ kịch bản Go-live</button><button data-act="adv-guide-reset">⟲ Reset tiến độ hướng dẫn</button></div></details></div></aside><div class="main"><header class="topbar"><button class="tb-menu-btn" data-act="sb-toggle" aria-label="Menu">${I('menu')}</button><div class="crumbs" id="crumbs"></div><div class="tb-search" data-act="palette">${I('search')}<input placeholder="Tìm kiếm phòng, khách thuê, hợp đồng..." readonly><span class="kbd">Ctrl</span><span class="kbd">K</span></div><button class="tb-guide" data-act="guide" data-guide="guide-btn">${I('book-open')}<span>Hướng dẫn thao tác</span></button><button class="tb-bell" data-act="bell" aria-label="Thông báo">${I('bell')}<span class="badge" id="bell-badge">0</span></button><div class="tb-period"><span>Kỳ báo cáo</span><select id="period-sel" data-on="period"></select></div><div class="tb-user" data-act="user-menu"><div class="avatar" id="tb-avatar"></div><div><div class="nm" id="tb-name"></div><div class="rl" id="tb-role"></div></div>${I('chevron-down')}</div></header><main class="content" id="content"></main></div></div>`;
    U.bind(app, {
