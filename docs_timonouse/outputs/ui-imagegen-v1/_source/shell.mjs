...
function sidebar(active) {
  const groups = NAV.map((g) => {
    ...
    const items = open
      ? `<ul>${g.items.map(([id, label]) => `<li class="${id === active ? 'on' : ''}">${label}</li>`).join('')}</ul>`
      : '';
    return `<div class="grp ${open ? 'open' : ''}"><div class="gh">${icon(g.icon, 16)}<span>${g.group}</span>${icon(open ? 'chevronDown' : 'chevronRight', 14, 'chev')}</div>${items}</div>`;
  }).join('');
  return `<aside class="sb">
    ...
    <nav>${groups}</nav>
    <div class="sbf">Mockup v1.6 · dữ liệu Seed v1.0<br/>Trục: G1 kỳ 09/2026 · đối soát 08/2026</div>
  </aside>`;
}

function topbar({ breadcrumb = [], period = 'Kỳ 09/2026', user = 'Admin demo', role = 'Quản trị viên', bell = 12 }) {
  const bc = ['Trang chủ', ...breadcrumb].map((b, i, a) => (i === a.length - 1 ? `<b>${b}</b>` : `<span>${b}</span>`)).join(`<i>/</i>`);
...
<body><div class="app">${sidebar(o.active)}<div class="col">${topbar(o)}<main class="${o.rail ? 'withrail' : ''}"><section class="content">${main}</section>${o.rail ? `<aside class="rail">${o.rail}</aside>` : ''}</main></div></div></body></html>`;
}
...
export function btn(text, kind = 'outline', ic) {
  return `<button class="btn ${kind}">${ic ? icon(ic, 15) : ''}${text}</button>`;
}
...
export function table(cols, rows, opts = {}) {
  ...
  const tr = rows.map((r) => {
    ...
    return `<tr class="${cls}">${cells.map((v, i) => `<td class="${cols[i]?.num ? 'n' : ''} ${cols[i]?.cls || ''}">${v ?? ''}</td>`).join('')}</tr>`;
  }).join('');
...
/** Tabs */
export function tabs(list, activeIdx = 0) {
  return `<div class="tabs">${list.map((t, i) => `<span class="${i === activeIdx ? 'on' : ''}">${t}</span>`).join('')}</div>`;
}
