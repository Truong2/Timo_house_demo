...
const expectedTopLevel = { admin: 9, qltong: 6, tpvh: 5, ops: 7, accountant: 7, sale: 4, kythuat: 3, hr: 3, codong: 2 };
...
assert.equal(routes.length, 84, 'Expected the demo route inventory to contain 84 routes');
assert.equal(new Set(routes.map(route => route.path)).size, 84, 'Route paths must be unique');
...
assert.equal(matrixCases, 9 * 84 * 2);
state.session = null;
