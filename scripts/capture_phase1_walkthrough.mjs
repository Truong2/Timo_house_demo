...
async function runScenario(page, base) {
  ...
  await run('F08.4', async () => { await clickText(page, 'Ghi nhận đã hoàn'); const m = await waitForModal(page); await fillDemo(page, 'refund-paid', m); await clickText(page, 'Xác nhận đã hoàn'); await page.waitForTimeout(350); const s = await stateSnapshot(page); const dedStatus = await page.evaluate((id) => (window.TH.store.state.refundDeductions || []).filter((d) => d.refundId === id).map((d) => d.status || '-'), s.refund?.id); const [download] = await Promise.all([page.waitForEvent('download'), clickAction(page, 'dl')]); const artifact = await saveDownload(download, { downloads: DOWNLOADS, out: OUT }, { expectValues: [{ row: 'any', value: s.refund?.code || '' }] }); return { actual: `Hồ sơ ${s.refund?.code} Đã hoàn 12.600.000đ; UNC289104 và uy_nhiem_chi_hoan_coc.pdf đã lưu; dòng khấu trừ [${dedStatus.join(', ')}].`, downloads: [artifact], assertions: [{ id: 'deduction-status-sync', status: dedStatus.length && dedStatus.every((x) => x === 'refunded') ? 'PASS' : 'FAIL', detail: dedStatus.join(', ') || 'no deductions' }] }; }, { before: true });
  await switchRole(page, 'ops');
