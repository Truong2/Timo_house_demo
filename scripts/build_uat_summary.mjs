...
const issueLines = [
  ...
  `- Baseline: develop@11cbdb6 (gồm luồng chủ nhà → HĐ đầu vào → tòa → phòng) · mã đã chạy: \`${BRANCH}@${COMMIT}\``,
...
const children = [
  ...
  h('Lỗi đã sửa trong nhánh UAT'), p('1) Đồng bộ trạng thái dòng khấu trừ với hồ sơ hoàn cọc (assert deduction-status-sync). 2) Chụp đúng dòng công nợ demo và số dư sau thu một phần (F05.4). 3) Kiểm tra nội dung file tải xuống: tên, dung lượng, header/số dòng, giá trị chính, SHA-256. 4) Smoke liên phase là assertion state thật thay vì ghi cứng.'), p('Lỗi sản phẩm phát hiện khi UAT và đã sửa: (a) Kinh doanh bấm “Xác nhận chốt thuê” bị đưa vào trang 403 vì không có quyền tạo HĐ → nay về chi tiết giao dịch, Vận hành tạo HĐ từ giao dịch; (b) hồ sơ hoàn cọc “Cần chỉnh sửa” thiếu nút Sửa phương án / Gửi duyệt; (c) nghĩa vụ góp vốn tạo từ đợt góp vốn bị gắn nguồn “auto” nên không nhận diện được bản ghi người dùng.'),
